...
test(
  "a card's title can be edited and the change persists",
  { tag: "@self-hosted" },
  async ({ page }) => {
    ...
    await board.createCard("Original title");
    await board.openCard("Original title");

    await card.editTitle("Updated title");
    await page.reload();
